...
//           setTimeout(() => setIsLoading(false), 250); //500
//           return 80;//80
//         }
//         return prevProgress + Math.random() * 18;  //15
//       });
//     }, 120);//200
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 80) {
          clearInterval(interval);
          setTimeout(() => setIsLoading(false), 300); // fast exit
          return 80;
        }
      ...
      >
        ...
        >
          ...
          <motion.h2
            ...
            className="text-white font-['Space_Grotesk'] text-2xl font-semibold mb-8"
          >
            {progress < 70 ? "Loading..." : "Almost there"}
          </motion.h2>
          ...
          {/* Progress percentage */}
          <motion.p className="mt-4 text-[#00CFFF] font-['Inter'] font-semibold text-sm">
            {progress < 80 ? `${Math.floor(progress)}%` : "Ready"}
          </motion.p>
        </motion.div>
